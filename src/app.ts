...
import { UserPendentingRentError } from "./errors/user-pendenting-rent-error";
...
export class App {
    ...
    async removeUser(email: string): Promise<void> {
        const rents = await this.rentRepo.findOpenRentsFor(email)
        if(rents.length > 0) throw new UserPendentingRentError()
