...
import { UserPendentingRentError } from "../src/errors/user-pendenting-rent-error";
...
describe('App', () => {
    ...
    it('should throw user pendenting rent error when trying to remove user with rent in your user', async () => {
        const app = new App(userRepo, bikeRepo, rentRepo)
        const userEmail = 'jose@mail.com'
        const user = new User('Jose', userEmail, '1234')
        await app.registerUser(user)
        const bike = new Bike('caloi mountainbike', 'mountain bike', 1234, 1234, 100.0, 'My bike', 5, [])
        app.registerBike(bike)
        await app.rentBike(bike.id, user.email)
        await expect(app.removeUser(userEmail)).rejects.toThrow(UserPendentingRentError)
    })
})
